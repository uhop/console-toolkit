...
test('Styling with back quotes', t => {
  t.test('Plain styling', t => {
  ...
  t.test('Styling with RESET_STATE c vs. s', t => {
  ...
  t.test('Bold and dim', t => {
    const text = s`normal{{bold}}bold{{dim}}bold and dim{{reset.bold}}dim{{reset.dim}}normal`;
    t.equal(text, 'normal\x1B[1mbold\x1B[2mbold and dim\x1B[22;2mdim\x1B[22mnormal');
  });
});
